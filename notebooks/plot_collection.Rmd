...
#ggplot(df, aes(x = PC3, y = ratio)) +
#  geom_point(size = 1.8, alpha = 0.7) +
#  geom_line(data = cbind(x_line2, ratio = y_line), linewidth = 1,
#            aes(y = ratio)) +
#  scale_y_log10() +
...
```{r}
# Remove K most extreme in log10(ratio) using MAD, then refit
K <- 1
...
p_A <- ggplot(df2, aes(x = PC3, y = ratio)) + # <--- ASSIGNED TO p_A
  ...
  theme_minimal()
print(p_A) # Print to display the plot in the Rmd output
...
#cat("Good Hβ measurements kept:", nrow(hb), "\n")
...
p_B <- ggplot(df1, aes(x = x_pred, y = .data[[Y_COL]])) + # <--- ASSIGNED TO p_B
  ...
  theme_minimal()
print(p_B) # Print to display the plot in the Rmd output

```
```{r}
# reuse z_pop from above
...
p_C <- ggplot(df2, aes(x = x_pred, y = .data[[Y_COL]])) + # <--- ASSIGNED TO p_C
  ...
  theme_minimal()
print(p_C) # Print to display the plot in the Rmd output

```

```{r make_composite_panel, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=7.25}
M   <- flux_matrix
lam <- wavelengths
...
print(p_D) # Print to display the plot in the Rmd output
...
```{r make_composite_panel, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=7.25}
# Loadings (unit-length eigenvectors) and PC std devs
...
# --- Plot in original flux scale (no stacking) --------------------------------
...
```{r make_composite_panel, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=10}
library(patchwork)
